...
Os conteúdos mantidos em `/Markdowns` foram incorporados na nova estrutura de `/SDD`, sem perder a reprodução do contexto original. As regras, requisitos e planos do legado foram reorganizados conforme a responsabilidade do projeto:

- `/Markdowns/AI-agent-SDD.md` e `/Markdowns/AI-Agent-Full-Prompt.md` foram abrangidos pelos contextos em `application.md`, `domain.md`, `database.md`, `repository.md`, `http.md`, `di.md`, `mensageria.md`, `autenticacao.md` e `idempotencia.md`.
- `/Markdowns/next-plan.md` e `/Markdowns/planejamento-faltantes-test.md` foram consolidados em `next-plan.md` dentro do diretório legado e em partes na base documental dos novos contextos.
- `/Markdowns/test.md` permanece como referência de critério de avaliação e foi preservado em `/SDD/test.md` para continuidade operacional.
- `/Markdowns/Agent.md` foi mantido como referência de execução do agente, com a versão ativa em `/SDD/Agent.md`.
...
1. Antes de escrever código, leia primeiro os contextos de domínio, aplicação, banco, idempotência e testes; isso reduz erros de arquitetura e de regras financeiras.
2. Implemente em sequência: domínio puro, persistência, casos de uso, HTTP, mensageria e testes de integração. Não pule etapas para preservar a confiabilidade do fluxo.
3. Para cada requisito, defina primeiro o contrato, depois a implementação e, por fim, os testes de aceitação. Isso ajuda o agente a manter clareza sobre entradas, saídas e invariantes.
4. Sempre valide o comportamento sob concorrência, replays, falhas de infraestrutura e referências pendentes; esses são os pontos mais sensíveis do desafio.
5. Use `go test`, `go test -race` e simulações de integração para confirmar que a solução respeita transações, saldo e ledger sem perdas.
6. Em código e documentação, priorize rastreabilidade: identifique por que a solução é segura, como idempotência funciona e como o banco reforça invariantes financeiros.
7. Mantenha a solução orientada a evidência: cada regra deve ter teste, documentação e implementação que correspondam ao mesmo modelo de domínio.
